// CVEDB Feature changelog from v4 to v5 (cvedb010 branch)
//
...
// - Has a non-auth /publicjson/CVE-id endpoint to grab the (full) JSON of only public issues (not for public use, for CVEDB etc)
...
async function cvedbemaillists (req, res) {
    var emaillist = await new Promise( xres => { self.getemaillistforpmc(req.query.pmc, xres)});    
    res.send(emaillist)
}

async function cvedbpublicjsonlist(req, res) {
...
async function cvedbpublicjson(req, res) {
...
function cvedblogout (req, res) {
    req.logout();
    req.session.returnTo = null;
    res.redirect('/users/login');    
}

function cvedblogin (req, res) {
    sess = req.session;
    if (req.query.code) {
...
var self = module.exports = {
    ...
    cvedbroutes: function (ensureAuthenticated, app) {
        app.get("/users/login", cvedblogin); // replaces existing
        app.get("/users/logout", cvedblogout); // replaces existing        
        ...
        app.get('/cvedbemaillists', ensureAuthenticated, cvedbemaillists); // work around CORS
        app.get('/publicjson', cvedbpublicjsonlist);
        app.get('/publicjson/:id', cvedbpublicjson);
    },

    cvedbgroupacls: function (documentacl,yourpmcs) {
	//console.log('cvedb9 doc owner is '+documentacl+" and you are "+yourpmcs);
	...
	//console.log('cvedb9 access denied');
	return false;
    },

    // When a CVE record is changed this hook is called
    
    cvedbhookupsertdoc: function(req,dorefresh) {
	...
	// enforce workflow state cve4
        if (req.body.CVE_data_meta) { // CVE 4.0
		    ...
		    console.log("cvedb4 reserved but the description changed");
        ...
        if (req.body.CNA_private && req.body.CNA_private.state) { // CVE 5.0
		    ...
		    console.log("cvedb4 RESERVED but saved by security, no change");
                }
	    }
        }        
    },

    cvedbhookshowcveacl: function(doc, req, res) {
        if (!doc) {
	...
	if (doc && doc.body && doc.body.CNA_private && doc.body.CNA_private.owner) {
	    if (!self.cvedbgroupacls(doc.body.CNA_private.owner, req.user.pmcs)) {
    ...
    cvedbhookaddcomment: function(doc,req) {
        var pathcve = "cve";
    ...
    cvedbhookaddhistory: function(oldDoc, newDoc) {
	if (oldDoc != null) {
            if (newDoc.body.CVE_data_meta) { // CVE 4.0
	        if (newDoc.body.CVE_data_meta.STATE != oldDoc.body.CVE_data_meta.STATE) {
		    console.log("cvedb4 changed state "+newDoc.body.CVE_data_meta.STATE);
		    ...
		    console.log("cvedb4 changed state "+newDoc.body.CNA_private.state);
    ...
    cvedballowedtodelete: function(req) {
