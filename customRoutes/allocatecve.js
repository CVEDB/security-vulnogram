...
protected.get('/', csrfProtection, function (req, res) {
    ...
    if (req.user.pmcs && req.user.pmcs.length == 1) {
        if (!allowedtoallocatelive(req.user.pmcs,req.user.pmcs[0])) {
            title = "This form will request a CVE from the CVEDB Security team by email.";
        }
    } else if (!req.user.pmcs || !req.user.pmcs.includes(conf.admingroupname)) {
        title = "This form will request a CVE either from the CVEDB Security team, or if your PMC is allowed, direct from the CVE project.  In either case an email will be sent to the security team and the PMC.";
    }
...
protected.post('/', csrfProtection, async function(req,res) {
    ...
    var eto = cvedb.getsecurityemailaddress(pmc);
    if (pmc =="security" || testmode) {
        eto = "security@akhulnasoft.com";
    }
    ...
    var emaillist = await new Promise( res => { cvedb.getemaillistforpmc(req.body.pmc, res)});
